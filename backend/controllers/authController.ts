...
export const register = asyncHandler(async (req: Request, res: Response) => {
  ...
  const userData = { name, email, password: hashedPassword, role: 'user' as const }; // Rol por defecto 'user'
  ...
  const createdUser = await usersService.getById(result.lastInsertRowid as number);
  if (!createdUser) {
...
export const login = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!isMatch) {
    ...
    let updateData: Partial<User> = { failedLoginAttempts: newFailedAttempts };
 ...
 * @access  Private/Admin
 */
export const deleteOldAuditLogs = asyncHandler(async (req: Request, res: Response) => {
...
export const refreshToken = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const decoded = jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET) as { id: string | number };
